...
var authConfig = {
   ...
   * [pass]: Basic Auth 的密碼
   * [protect_file_link]: Basic Auth 是否用於保護文件連結，默認值（不設置時）為 false，即不保護文件連結（方便 直連下載/外部播放 等）
   * 每個盤的 Basic Auth 都可以單獨設置。Basic Auth 默認保護該盤下所有文件夹/子文件夹路徑
   * 【注意】默認不保護文件連結，這樣可以方便 直連下載/外部播放;
   ...
   * 【注意】對於id設置為子文件夹id的盤將不支持搜尋功能（不影響其他盤）。
   */
  roots: [
    {
      id: "",
   ...
   * 如果在全局認證的基礎上，仍需要給某些目錄單獨進行 .password 文件內的密碼驗證的話，將此選項設定為 true;
